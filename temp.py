...
def left_panel(surface):
    player_name_lbl = heading3.render('Player Name: ', True, (0, 0, 0))
    player_name_lbl_rect = player_name_lbl.get_rect(
        topleft=(GRID_WIDTH * 1.25, GRID_HEIGHT * 21))
    surface.blit(player_name_lbl, player_name_lbl_rect)

    display_high_score(highScoreSurface, surface)

    start_text = heading3.render('Start', True, pygame.Color("black"))
    if startGame:
        pygame.draw.rect(surface, "lightslategrey", sg_bg)
    pygame.draw.rect(surface, (255, 255, 255), pn_bg)
    player_name_surface = paragraph.render(player, True, (0, 0, 0))
    surface.blit(player_name_surface, pn_bg)
    if startGame:
        surface.blit(start_text, (GRID_WIDTH * 7.9, GRID_HEIGHT * 26))
